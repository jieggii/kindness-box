...
class HomeKeyboardNoRecipients(Keyboard):
    EDIT_MY_LIST = "Выбрать людей"
    REJECT_PARTICIPATION = "Отказаться от участия"

    def __init__(self):
        super(HomeKeyboardNoRecipients, self).__init__()
        self.add_text_button(text=self.EDIT_MY_LIST, color=ButtonColor.PRIMARY)
        self.add_row()
        self.add_text_button(text=self.REJECT_PARTICIPATION, color=ButtonColor.SECONDARY)


class ConfirmGiftsDeliveryKeyboard(BoolKeyboard):
    TRUE = "Да, всё верно"
    FALSE = "Отмена"

    def __init__(self):
        super(ConfirmGiftsDeliveryKeyboard, self).__init__(primary_option=False)
