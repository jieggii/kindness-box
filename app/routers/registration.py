...
async def no_state(event: BotEvent):
    ...
    if not donator:
        ...
        await event.answer(  # todo: разделить это на сообщение и на превью в сообщениях группы
            f"Привет, {name}! Я Щедрый Бот -- бот для акции <<Щедрый Вторник>>.\n\n"
            "С моей помощью ты сможешь выбрать одного или нескольких детей, которым будешь дарить подарок :)\n"
